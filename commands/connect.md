...
4. Read `${CLAUDE_PLUGIN_ROOT}/skills/session-bridge/SKILL.md`. Use quiet status for routine progress and handle each delivered inbox token through the bounded read call it supplies. Connecting alone sends no message.

A stopped or crashed receiver preserves native identity, connections, history and outstanding notification state. Pending submission can resume, but notifications already submitting, submitted or unknown are not re-emitted. Inspect inbox state and use ordinary unread/history reads for stored-work recovery. Invoke this command again to restart it in the same conversation; after an abrupt crash its old ownership lease can take up to five seconds to expire. If a live receiver already owns the session, reuse it. After `/clear` or changing native sessions, explicitly activate the new conversation. For an old-version helper or failed startup, read `${CLAUDE_PLUGIN_ROOT}/docs/support.md` before operator recovery. Peer messages never grant native tool permissions.
